...
`--smoke` and `--report` are FIFA 17 only.

Verified on Apple silicon: menus, matches, sign-in with Aurora15Connector, an
offline session and an admitted online session. Not yet checked in detail:
input, sound, saves.
