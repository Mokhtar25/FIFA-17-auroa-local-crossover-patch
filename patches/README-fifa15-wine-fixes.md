...
Patches against `crossover-sources-26.3.0`. The combined `main` package
includes the FIFA 15 work; `build.sh` applies
...
and builds `gdiplus.dll`, which both profiles install in the shared app (the launcher's exit crash,
...
(`fixes/x86_64-windows/gdiplus.dll`); FIFA 17 setups also install gdiplus so refreshing the shared app keeps this fix. Upstream-worthy.
